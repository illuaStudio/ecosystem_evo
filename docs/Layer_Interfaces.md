...
| 関数 | 用途 |
|------|------|
| `get_phase_view(controller, world)` | フェーズ・ウェーブ・ストーリー HUD |
| `should_advance_sim(controller)` | ストーリー中は False（Sim tick を止める） |
| `acknowledge_story(controller)` | ストーリー続行 |
| `request_start_defense(controller)` | 開発→防衛の手動開始 |
...
|--------|------|------|
| 高 | import 契約テスト | ✅ 実施済み |
| 高 | `client_api` への Client 依存集約 | 随時（Client 変更時）。詳細は docs/Client_Game_Layer_Boundary_for_Parallel_AI_Development.md |
...
- `docs/Game_Vision.md` — **ゲーム企画・フェーズ・ロードマップ（Game 層開発の参照）**
- `docs/Client_Game_Layer_Boundary_for_Parallel_AI_Development.md` — **Client層とGame層の並行開発境界（異なるAIがそれぞれ触るための実践ガイド）**
