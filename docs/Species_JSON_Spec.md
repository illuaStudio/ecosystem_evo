...
このファイルは全生物（Ant, Spider, 微小動物 など）の `config/species/*.json` の構造と各パラメータの意味を定義します。
...
| ファイル名 | 任意（例: `springtail.json`）。ゲーム内の識別は JSON 内の **`name`** フィールドで行う |
...
| キー | 型 | 必須 | 意味 |
|------|----|------|------|
| name | string | **必須** | 種族名。`CreatureFactory` / `Species.create("springtail")` などで参照する ID |
...
| キー | 型 | 意味 | 例 |
|------|----|------|----|
| mature | int | 成熟年齢。`life_cycle.mature` を参照する繁殖が可能になる境界 | 280 |
...
| corpse_decompose_rate | float | **死骸の自然分解速度**。`CorpseComponent.update(dt)` で `initial_biomass × rate × dt` だけ `remaining_biomass` が減少 | 0.00001〜0.01 | 0.00003 |
...
| キー | 型 | 意味 | デフォルト |
|------|----|------|-----------|
| target_type | string | 獲物の `species.name`（例: `"springtail"`） | — |
...
| food_leak_per_tick | float | 予約備蓄を超えた余剰があるとき、1 シミュ tick で失われる食料量 | 0.5 |
...
**C 案（現状）:** 持ち帰りは **食料（バイオマス）** として `stored_food` に蓄える。コロニーは `FeedAtNest` で満腹度に変換。余剰は `food_leak_per_tick` で消失する。
...
| キー | 型 | 意味 | デフォルト |
|------|----|------|-----------|
| target_type | string | 獲物の種名（単一種） | — |
| target_types | string[] | 複数種を狩る場合（`target_type` より優先）。例: `["springtail", "Spider"]` | — |
...
### springtail（微小動物・成長・寿命）

`config/game/species/springtail.json` を参照。

### Ant（コロニー・狩り・持ち帰り・巣で食事）

`config/game/species/red_ant.json` などを参照。満腹時は **微小動物 + Spider** を狩って巣へ貯蔵。飢餓時のみ巣食事・その場食事を優先。

### Spider（アリ捕食・大型獲物）

`config/game/species/spider.json` を参照。高 HP・大サイズ。空腹時は `ChaseAction` で **アリ系** を追跡し、接触時に `try_predate`（殺害→その場で `consume_carcass`）。巣・コロニーなし。満腹時は `WanderAction`。
