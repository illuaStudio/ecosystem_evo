...
`config/sim/species/springtail.json` を参照。

### Ant（コロニー・狩り・持ち帰り・巣で食事）

`config/sim/species/red_ant.json` などを参照。満腹時は **微小動物 + Spider** を狩って巣へ貯蔵。飢餓時のみ巣食事・その場食事を優先。

### Spider（アリ捕食・大型獲物）

`config/sim/species/spider.json` を参照。高 HP・大サイズ。空腹時は `ChaseAction` で **アリ系** を追跡し、接触時に `try_predate`（殺害→その場で `consume_carcass`）。巣・コロニーなし。満腹時は `WanderAction`。
