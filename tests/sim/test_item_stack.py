...
class TestItemStack(unittest.TestCase):
    def test_biomass_capacity_deposit_withdraw(self):
        stack = ItemStack.from_biomass_capacity(100.0, 10.0)
        self.assertAlmostEqual(stack.biomass_amount(), 10.0)
        self.assertAlmostEqual(stack.deposit_biomass(50.0), 50.0)
        self.assertAlmostEqual(stack.biomass_amount(), 60.0)
        self.assertAlmostEqual(stack.deposit_biomass(100.0), 40.0)
        self.assertAlmostEqual(stack.withdraw_biomass(25.0), 25.0)
        self.assertAlmostEqual(stack.biomass_amount(), 75.0)

    def test_object_storage_compat_api(self):
        storage = ObjectStorage(stored_food=30.0, max_food=200.0)
        self.assertAlmostEqual(storage.stored_food, 30.0)
        self.assertAlmostEqual(storage.max_food, 200.0)
        self.assertAlmostEqual(storage.deposit(10.0), 10.0)
        self.assertAlmostEqual(storage.stored_food, 40.0)
        self.assertAlmostEqual(storage.food_ratio, 0.2)
    ...
    def test_multi_slot_storage_config(self):
        ...
        self.assertAlmostEqual(storage.max_food, 100.0)
...
class TestItemTransfer(unittest.TestCase):
    def test_creature_to_storage_and_back(self):
        ...
        withdrawn = withdraw_biomass_from_parent(ant, 15.0)
        self.assertAlmostEqual(withdrawn, 15.0)
        self.assertAlmostEqual(root.storage.stored_food, 25.0)
    ...
    def test_linked_chests_share_item_stack(self):
        ...
        storage.stack.deposit_biomass(50.0)
        ...
        taken = withdraw_biomass_from_parent(ant, 20.0)
